...
BASE = r" C:\Users\shiva\.gemini\antigravity\scratch\skill-assessment-portal\backend\app\

# ----------------- SCHEMAS -----------------
schemas_code = '''from pydantic import BaseModel, EmailStr, Field
...
class AssessmentSessionOut(BaseModel):
 ...
 current_section: str
 strike_count: int
 questions: Dict[str, List[QuestionPublic]] # grouped by section
 saved_answers: Dict[int, Dict[str, Any]] # question_id -> {selected_option, code_submission, text_response, language}
...
class IntegrityEventCreate(BaseModel):
    session_id: str
    event_type: str # tab_switch, fullscreen_exit, copy_attempt, paste_attempt, cut_attempt, restricted_key, devtools_detected, context_menu
...
class SkillScoreDetail(BaseModel):
    ...
    level: str # Master, Proficient, Developing, Beginner
    category: str # Core Technical, Problem Solving, Analytical, Systems
...
class AssessmentResultOut(BaseModel):
    ...
    max_score: float
    percentage: float
    performance_band: str # Elite, Ready for Placement, Needs Bridge Training, Foundational
...
with open(os.path.join(BASE, 'schemas.py'), 'w', encoding='utf-8') as f:
